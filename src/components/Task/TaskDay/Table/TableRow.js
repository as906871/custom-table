...
const TableRow = ({ row, index, columns, onCellChange, onDeleteRow, isLastRow, onAddRow, onReorderRows }) => {
  ...
  return (
    ...
    >
      <td className="sticky left-0 z-0 bg-white hover:bg-gray-50 px-2 sm:px-3 lg:px-4 py-2 sm:py-3 text-xs sm:text-sm text-gray-600 font-medium border-r border-gray-200 shadow-sm">
        <div className="flex items-center justify-between gap-1">
          <div className="flex items-center gap-1">
            <GripVertical size={14} className="text-gray-400 cursor-move flex-shrink-0 sm:w-3.5 sm:h-3.5" />
            <span>{index + 1}</span>
          </div>
